...
public:
    ...
    , fixed_num_syns_( fixed_num_syns )
  {
    ...
    for (int i=0; i<prop_names.size(); i++)
        syn_datasets.push_back(new H5Dataset( this, prop_names[i].c_str() ));
    ...
    void iterateOverSynapsesFromFiles( NESTSynapseList & synapses )
    {
      ...
      for (int i=0; i<syn_datasets.size(); i++) {
          ...
          H5View memspace_view( count, i, synapses.num_params_ + 1 );
          ...
          if ( dataspace_view.count[ 0 ] > 0 )
          {
            ...
            hsize_t pool_size = synapses.property_pool_.size()/sizeof(int);
