...
class Tets {
  public:
    ...
    inline IntType get_n_tets() const { return n_tets_; }
    ...
    inline FloatType& molecule_occupancy_last_update_time(IntType s,
                                                          IntType i) {
        assert(s >= 0 && s < n_species_);
        assert(i >= 0 && i < n_tets_);
    ...
    void distribute_molecules(IntType species_idx, IntType n_molecules_tot,
        ...
        //        ---------------\n", species_idx); FloatType err_distr =
    ...
    inline void add_to_bucket(IntType tet_idx, IntType neighb_idx,
                              IntType diffusing_count) {
