...
class RdSolver {
  public:
    ...
    inline void recompute_propensities_after_reac(IntType r, IntType i) {
        std::vector<IntType> dependencies_idxs =
    ...
    // recompute propensities affected by diffusion of s-th species in i-th
    // tetrahedron
    ...
    void run_diffusions(FloatType tau) {
        ...
        for (IntType s = 0; s < model_.get_n_species();
             ++s) { // iterate through each species that has to diffuse
    ...
    inline void update_occupancies_at_reac(IntType r, IntType i,
                                           FloatType t_now) {
