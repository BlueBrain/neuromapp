...
class Model {
  public:
    ...
    inline FloatType get_n_species() const { return n_species_; }

    // return number of reactions
    inline FloatType get_n_reactions() const { return n_reactions_; }
    ...
    inline void apply_reaction(IntType r, IntType i,
    ...
    inline FloatType
    compute_reaction_propensity(IntType r, IntType i,
                                readi::Tets<IntType, FloatType>& tets) const {
    ...
    void read_from_file(std::string const& filename_model) {
        ...
        try {
            ...
            for (IntType s = 0; s < n_species_; ++s) {
                f >> discard >>
                    diffusion_coeff(s); // read coefficients of diffusion
            }
        ...
        for (IntType s = 0; s < n_species_; ++s) {
            ...
            for (IntType r_dep = 0; r_dep < n_reactions_; ++r_dep) {
                if (std::find(reactions_[r_dep].lhs_idxs_.begin(),
                              reactions_[r_dep].lhs_idxs_.end(),
                              s) != reactions_[r_dep].lhs_idxs_.end())
                    dependencies.push_back(r_dep);
            }
