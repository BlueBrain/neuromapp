...
/**
 * @file neuromapp/coreneuron_1.0/event_passing/drivers/main.cpp
...
int qhelp(int argc, char* const argv[], po::variables_map& vm){
    ...
    ("numspike,s", po::value<size_t>()->default_value(30),
   ...
   if( (vm["num-ite"].as<int>() + vm["num-local"].as<int>()
   + vm["num-local"].as<int>()== 0)){
 ...
 */
void events_miniapp(po::variables_map const& vm){
