...
#ifndef MAPP_RL_MPIIO_H
#define MAPP_RL_MPIIO_H
...
class MPIIOWriter : public replib::Writer {
 ...
 */
MPIIOWriter::MPIIOWriter() : f_(NULL) {}


/** \fun ~MPIIOWriter()
    \brief Class destructor */
MPIIOWriter::~MPIIOWriter() {
    delete(f_);
}
...
void MPIIOWriter::init(replib::config &c) {
    if (c.write() == "file1b") {
        f_ = file1b(c);
...
void MPIIOWriter::open(mapp::timer &t_io, const std::string & path) {
    // Get the file path
...
inline void MPIIOWriter::write(float * buffer, size_t count) {
    MPI_Status status;
...
void MPIIOWriter::write(mapp::timer &t_io, float * buffer, size_t count) {
    MPI_Status status;
...
void MPIIOWriter::close(mapp::timer &t_io) {
    t_io.tic();
    close();
    t_io.toc();
}
...
unsigned int MPIIOWriter::total_bytes() {
    return f_->total_bytes();
}

} // end namespace

#endif // MAPP_RL_MPIIO_H
