...
namespace neuromapp {
    ...
    template <value_type,allocator_type>
    /*function for taking in an array of floats ( TODO make generic) 
     * and creating an unsigned int array that represents its parts
     */
    void convert_to_parts(pointer row_ptr,block<unsigned int,allocator_type> & split_block,size_type row_num){
     ...
     */
    void convert_from_parts(pointer row_ptr,block<unsigned int,allocator_type> & split_block,size_type row_num){
        size_type count = split_block.dim0()/3;
        for (size_type i = 0 ; i < count ; i++) {
            conversion_parts conv_float;
