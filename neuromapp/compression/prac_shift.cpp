...
vector<std::string> csv_bulk_fnames = {"../compression/trans_data/visually_easy.csv"};
...
int main (void) {
    ofstream out("part_splitting_time.log");
    ...
    for (string fname : csv_bulk_fnames) {
        ...
        std::cout << "split block is " << split_block << std::endl;
        //recombine the split block, and compare with the original
        prac_block b1_comp = neuromapp::generate_unsplit_block(split_block);
        if ( b1_comp == b1) {
            std::cout << "yup they compared as equal" << std::endl;
            std::cout << "b1comp is "<< b1_comp << std::endl;
