...
bool full_col_persistence(IT r1_s,IT r2_s,IT r1_ns,IT r2_ns,sz cols_to_compare) {
    while(cols_to_compare > 0) {
        ...
        if (*r2_temp_cpy != *r2_ns) return false;
        r1_s++;
        r2_s++;
        r1_ns++;
        r2_ns++;
...
BOOST_AUTO_TEST_CASE_TEMPLATE(sort_test,T,test_allocator_types){
    ...
    for (vector<string> fname_container : {bulk_files}) {
        for (string fname : fname_container) {
            ...
            BOOST_CHECK_MESSAGE(true == full_col_persistence(st_srow_s,st_other_s,st_srow_ns,st_other_ns,cols_),
