...
BOOST_AUTO_TEST_CASE(reac_constructor1_test){
    ...
    readi::Reac<int, double> reac(spec_names, lhs, rhs, reaction_rate);
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(lhs_idxs.begin(), lhs_idxs.end(), reac_lhs_idxs.begin(), reac_lhs_idxs.end());
    BOOST_CHECK_EQUAL_COLLECTIONS(lhs_cnts.begin(), lhs_cnts.end(), reac_lhs_cnts.begin(), reac_lhs_cnts.end());
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(rhs_idxs.begin(), rhs_idxs.end(), reac_rhs_idxs.begin(), reac_rhs_idxs.end());
    BOOST_CHECK_EQUAL_COLLECTIONS(rhs_cnts.begin(), rhs_cnts.end(), reac_rhs_cnts.begin(), reac_rhs_cnts.end());
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(upd_idxs.begin(), upd_idxs.end(), reac_upd_idxs.begin(), reac_upd_idxs.end());
    BOOST_CHECK_EQUAL_COLLECTIONS(upd_cnts.begin(), upd_cnts.end(), reac_upd_cnts.begin(), reac_upd_cnts.end());
...
BOOST_AUTO_TEST_CASE(reac_scaled_rate_test){
    ...
    BOOST_CHECK_CLOSE(reac.compute_c_mu(reaction_rate, volume), reaction_rate*std::pow(1.0e+3*volume*N_avogadro, -2), rel_tolerance);
}
