...
BOOST_AUTO_TEST_CASE(read_file_test) {
    ...
    readi::Mesh<unsigned int, double,
    ...
    std::vector<double> vols = {8.0e-19, 9.0e-19, 6.0e-19,
                                5.0e-19, 3.0e-19, 4.0e-19};
    for (int i = 0; i < n_tets; ++i)
        BOOST_CHECK_CLOSE(tets.volume(i), vols[i], rel_tolerance);
    BOOST_CHECK_CLOSE(tets.get_tot_volume(),
                      std::accumulate(vols.begin(), vols.end(), 0.),
                      rel_tolerance);

    std::vector<std::vector<int>> neighbs = {{1, 2, -1, -1}, {0, 4, -1, -1},
                                             {0, 3, -1, -1}, {2, 5, -1, -1},
                                             {1, 5, -1, -1}, {3, 4, -1, -1}};
    for (int i = 0; i < n_tets; ++i)
        for (int j = 0; j < 4; ++j)
